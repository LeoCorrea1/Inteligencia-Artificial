...
class Perceptron:
	...
	def teste(self, amostra):
		...
		if saida_gerada == 1:
			print('Classe: %d. Baixo risco ( APROVADO )' % saida_gerada)
		else:
			print('Classe: %d. Alto Risco ( RECUSADO )' % saida_gerada)
...
with open('TrabalhoLeonardo/amostras_saidas_problemaCredito.csv', 'r') as arquivo:
		leitor_csv = csv.reader(arquivo)
		for linha in leitor_csv:
			amostra = [float(linha[0]), float(linha[1])] 
			saida = int(linha[2])  
...
while (True):
	...
	print("\n(Normalizados de 0 a 1) Quanto mais próximo de 1, melhor o histórico e maior a renda\n")
	x = float(input('Valor para Historico de creditp: '))
